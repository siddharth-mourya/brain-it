...
export function PageWorkspace({ page }: { page: { id: string; title: string; content: any; whiteboard: any; updated_at: string } }) {
 ...
 useEffect(()=>{ const interval=setInterval(async()=>{ if(!editor) return; await (supabase.from('pages') as any).update({title, content: editor.getJSON(), whiteboard: wb, updated_at:new Date().toISOString()}).eq('id', page.id); },3000); return ()=>clearInterval(interval);},[editor,page.id,supabase,title,wb]);
 const uploadImage=async(file:File)=>{const path=`${page.id}/${Date.now()}-${file.name}`; const {error}=await supabase.storage.from('uploads').upload(path,file,{upsert:false}); if(error) return toast.error(error.message); const {data}=supabase.storage.from('uploads').getPublicUrl(path); editor?.chain().focus().setImage({src:data.publicUrl}).run(); await (supabase.from('page_images') as any).insert({page_id:page.id,user_id:(await supabase.auth.getUser()).data.user!.id,path,url:data.publicUrl});};
