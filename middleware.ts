...
export async function middleware(request: NextRequest) {
  ...
  const protectedRoute = request.nextUrl.pathname.startsWith('/dashboard') || request.nextUrl.pathname.startsWith('/page');
  if (!user && protectedRoute) return NextResponse.redirect(new URL('/login', request.url));
  if (user && (request.nextUrl.pathname === '/login' || request.nextUrl.pathname === '/signup')) return NextResponse.redirect(new URL('/dashboard', request.url));
  return response;
}
